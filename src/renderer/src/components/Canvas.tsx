      ...
      <div className="min-h-0 flex-1">
        {tab === 'preview' && (
          <div key="preview" className="anim-fade-in relative h-full w-full">
            ...
            {selectedFile && (
              <div className="absolute left-3 top-3 rounded-md bg-overlay/70 px-2 py-1 text-[11px] text-white backdrop-blur">
                {selectedFile}
                <button
                  onClick={() => setSelectedFile(null)}
                  className="ml-2 text-white/65 hover:text-white"
                >
                  ×
                </button>
              </div>
